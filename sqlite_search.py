...
# Copyright (C) 2023, 2026  Jordan Bussanich
...
class RowSearchResult:
    ...
    def __hash__(self) -> int:
        return hash((self.search_term, 
                     self.result, 
                     ','.join(self.column_names), 
                     ','.join(map(str, self.row)),
                     self.table_name))
    ...
    def __eq__(self, __value: object) -> bool:
        ...
        return (
            self.search_term == __value.search_term and
...
def search_sqlite(
        ...
        input_file: str
) -> tuple[list[TableSearchResult], list[RowSearchResult]]:
    ...
    for table in tables:
        for searcher in searchers:
            if searcher.search_cell(table):
                table_name_matches.add(
                    TableSearchResult(searcher.search_term, table)
                )
...
if __name__ == "__main__":
    ...
    for keyword in keywords:
        if arguments.regex:
            searchers.append(
                RegexCellSearcher(keyword, arguments.case_sensitive)
            )
        else:
            searchers.append(
                TextCellSearcher(keyword, arguments.case_sensitive)
            )
    ...
    if len(results[0]) > 0:
        ...
        table_output = zip(
            [r.search_term for r in results[0]], 
        ...
        if arguments.show_details:
            ...
            for table, matches in result.items():
                for keyword, count in matches.items():
                    print(
                        f"Found {count} instances of '{keyword}' in '{table}'."
                    )
